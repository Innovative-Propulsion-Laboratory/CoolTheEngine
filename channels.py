...
def generate_channels(profile_data, width_data, height_data, angle_data, wall_thickness,
    ...
    channel_width_list = PchipInterpolator(x_chamber_throat_exit, [width_inj, width_conv, width_throat, width_exit])(z_coord_list)
    channel_height_list = PchipInterpolator(x_chamber_throat_exit, [ht_inj, ht_conv, ht_throat, ht_exit])(z_coord_list)
    beta_list = PchipInterpolator(x_chamber_throat_exit, [beta_inj, beta_conv, beta_throat, beta_exit])(z_coord_list)
    ...
    xA_list = -(r_coord_list + wall_thickness) * np.sin(np.deg2rad(alpha_list))\
        - (channel_width_list / 2) * np.cos(np.deg2rad(alpha_list))
    yA_list = (r_coord_list + wall_thickness) * np.cos(np.deg2rad(alpha_list))\
        - (channel_width_list / 2) * np.sin(np.deg2rad(alpha_list))
    ...
    xC_list = -(r_coord_list + wall_thickness + channel_height_list) * np.sin(np.deg2rad(alpha_list))\
        + (channel_width_list / 2) * np.cos(np.deg2rad(alpha_list))
    yC_list = (r_coord_list + wall_thickness + channel_height_list) * np.cos(np.deg2rad(alpha_list))\
        + (channel_width_list / 2) * np.sin(np.deg2rad(alpha_list))
    xD_list = -(r_coord_list + wall_thickness + channel_height_list) * np.sin(np.deg2rad(alpha_list))\
        - (channel_width_list / 2) * np.cos(np.deg2rad(alpha_list))
    yD_list = (r_coord_list + wall_thickness + channel_height_list) * np.cos(np.deg2rad(alpha_list))\
        - (channel_width_list / 2) * np.sin(np.deg2rad(alpha_list))

    # We compute the list of channel center points
    x_center_list = -(r_coord_list + wall_thickness + channel_height_list / 2) * np.sin(np.deg2rad(alpha_list))
    y_center_list = (r_coord_list + wall_thickness + channel_height_list / 2) * np.cos(np.deg2rad(alpha_list))
    ...
    initial_fin_thickness = ((2 * np.pi * (r_coord_list + wall_thickness)) - (nbc * channel_width_list)) / nbc
    ...
    return channel_vertices, channel_centerline, channel_inclination, channel_ar_list, \
        ...
        hydraulic_diameter, initial_fin_thickness, \
