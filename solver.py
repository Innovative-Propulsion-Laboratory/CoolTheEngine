...
def solver(hotgas_data, coolant_data, channel_data, chamber_data, interp_funcs=None):
    """
    ...
    if interp_funcs is not None:
        interp_rho_func, interp_mu_func, interp_cp_func, interp_k_func, interp_Tsat_func = interp_funcs
    ...
    # Use Coolprop if the tables are not used
    if interp_funcs is None:
    ...
    # Otherwise use the 2D interpolation functions (x50 times faster)
    else:
        coolant_viscosity_list[-1] = interp_mu_func(coolant_inlet_pressure, coolant_inlet_temp)
        coolant_cond_list[-1] = interp_k_func(coolant_inlet_pressure, coolant_inlet_temp)
        coolant_cp_list[-1] = interp_cp_func(coolant_inlet_pressure, coolant_inlet_temp)
        coolant_density_list[-1] = interp_rho_func(coolant_inlet_pressure, coolant_inlet_temp)
        coolant_Tsat_list[-1] = interp_Tsat_func(coolant_inlet_pressure)
    ...
    for i in range(n_points - 1, -1, -1):
        ...
        if i > 0:
            if interp_funcs is None:
                ...
                coolant_viscosity_list[i-1] = interp_mu_func(new_coolant_pressure, new_coolant_temp)
                coolant_cond_list[i-1] = interp_k_func(new_coolant_pressure, new_coolant_temp)
                coolant_cp_list[i-1] = interp_cp_func(new_coolant_pressure, new_coolant_temp)
                coolant_density_list[i-1] = interp_rho_func(new_coolant_pressure, new_coolant_temp)
                coolant_Tsat_list[i-1] = interp_Tsat_func(new_coolant_pressure)
