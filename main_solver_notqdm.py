...
def mainsolver(hotgas_data, coolant_data, channel_data, chamber_data):
    ...
    coolant_viscosity_list = [flp.viscosity(
        init_coolant_pressure, init_coolant_temp, fluid)]
    coolant_cond_list = [flp.conductivity(
        init_coolant_pressure, init_coolant_temp, fluid)]
    coolant_cp_list = [flp.cp(init_coolant_pressure, init_coolant_temp, fluid)]
    coolant_density_list = [flp.density(
        init_coolant_pressure, init_coolant_temp, fluid)]
    ...
    """with tqdm(total=nb_points_channel,
        ...
        for i in range(0, nb_points_channel):
            ...
            hotgas_visc, hotgas_cp, hotgas_cond, hotgas_prandtl = t.hotgas_properties(hotgas_recovery_temp_list[i],
                                                                                      molar_mass,
                                                                                      gamma_list[i])
            ...
            while abs(new_coldwall_temp - coldwall_temp) > 0.1 and abs(new_hotwall_temp - hotwall_temp) > 0.1:
                ...
                qW = 5.74 * ((PH2O[i] * y_coord_avec_canaux[i]) / 1e5) ** 0.3 * (hotgas_recovery_temp_list[i] / 100) ** 
                3.5
                qC = 4 * ((PCO2[i] * y_coord_avec_canaux[i]) / 1e5) ** 0.3 * (hotgas_recovery_temp_list[i] / 100) ** 3.5
                qRad = qW + qC

                # Computing the heat flux and wall temperatures (Luka Denies)
                flux = (hotgas_recovery_temp_list[i] - coolant_temp_list[i] + qRad / hg) / (
                        1 / hg + 1 / hl_cor + wall_thickness[i] / wall_cond)
                new_hotwall_temp = hotgas_recovery_temp_list[i] + (qRad - flux) / hg
                new_coldwall_temp = coolant_temp_list[i] + flux / hl

                # Compute new value of sigma (used in the Bartz equation)
                T_hotgas_throat = hotgas_recovery_temp_list[index_throat]
    ...
    for i in range(0, nb_points_channel):
        ...
        # Reynolds number of the coolant
        Re_cool = (v_cool * Dhy *
                   coolant_density_list[i]) / coolant_viscosity_list[i]
        coolant_reynolds_list.append(Re_cool)

        # Prandtl number of the coolant
        Pr_cool = (coolant_viscosity_list[i] *
                   coolant_cp_list[i]) / coolant_cond_list[i]
        ...
        if i == nb_points_channel - 1:
            # Distance between current point and the previous (Pythagoras)
            dl = ((xcanaux[i - 1] - xcanaux[i]) ** 2 +
                  (ycanaux[i - 1] - ycanaux[i]) ** 2) ** 0.5
        else:
            # Distance between current point and the next (Pythagoras)
            dl = ((xcanaux[i + 1] - xcanaux[i]) ** 2 +
                  (ycanaux[i + 1] - ycanaux[i]) ** 2) ** 0.5
        ...
        while abs(new_coldwall_temp - coldwall_temp) > 0.1 and abs(new_hotwall_temp - hotwall_temp) > 0.1:
            ...
            # Compute radiative heat transfer of H2O (W) and CO2 (C) (Luka Denies)
            qW = 5.74 * ((PH2O[i] * y_coord_avec_canaux[i]) /
                         1e5) ** 0.3 * (hotgas_temp_list[i] / 100) ** 3.5
            qC = 4 * ((PCO2[i] * y_coord_avec_canaux[i]) /
                      1e5) ** 0.3 * (hotgas_temp_list[i] / 100) ** 3.5
            qRad = qW + qC
            ...
            wall_cond = t.conductivity(
                Twg=new_hotwall_temp, Twl=new_coldwall_temp, material_name=material_name)
        ...
        if new_coolant_pressure < 0:
            raise ValueError("Negative pressure ! Pressure drop is too high.")
        new_cool_visc = flp.viscosity(
            P=new_coolant_pressure, T=new_coolant_temp, fluid=fluid)
        new_cool_cond = flp.conductivity(
            P=new_coolant_pressure, T=new_coolant_temp, fluid=fluid)
        new_cool_cp = flp.cp(P=new_coolant_pressure,
                             T=new_coolant_temp, fluid=fluid)
        new_cool_dens = flp.density(
            P=new_coolant_pressure, T=new_coolant_temp, fluid=fluid)
        new_cool_sound_spd = flp.sound_speed(
            P=new_coolant_pressure, T=new_coolant_temp, fluid=fluid)
