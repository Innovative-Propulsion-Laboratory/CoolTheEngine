...
def canaux(plagex,plagey,nbc,lrg,lrg_c,lrg_div,ht,ht_c,ht_div,tore,debit_total,n1,n2,n3,n4,e_col,e_div,e_c,n5,n6,lrg_c2,ht_c2):
    crx=csv.reader(open(plagex,"r"))    #ouverture des x
    cry=csv.reader(open(plagey,"r"))    #ouverture des y
    x_value=[] #en m
    y_value=[] #en m
    ...
    for i in range (0,len(xcanauxre),1):
        if i==0:
            ...
            vect1=(xcanauxre[i]-xcanauxre[i-1])/((((ycanauxre[i]-ycanauxre[i-1])**2)+((xcanauxre[i]-xcanauxre[i-1])**2))**0.5)
            vect2=(xcanauxre[i+1]-xcanauxre[i])/((((ycanauxre[i+1]-ycanauxre[i])**2)+((xcanauxre[i+1]-xcanauxre[i])**2))**0.5)
    ...
    for i in range (0,len(xcanauxre),1):
        if i==0:
            ...
            vect2=(xcanauxre[i]-xcanauxre[i-1])/((((ycanauxre[i]-ycanauxre[i-1])**2)+((xcanauxre[i]-xcanauxre[i-1])**2))**0.5)
            angle2=degrees(acos(vect2))
            angulaire.append(angle)
        else:
            vect2=(xcanauxre[i]-xcanauxre[i-1])/((((ycanauxre[i]-ycanauxre[i-1])**2)+((xcanauxre[i]-xcanauxre[i-1])**2))**0.5)
    ...
    veritas=[]
    for i in range (0,len(xcanauxre),1):
        verifepe=(((ycanauxre[i]-y_value[i])**2)-(sin(radians(angulaire[i]))*(ycanauxre[i]-y_value[i]))**2)**0.5
